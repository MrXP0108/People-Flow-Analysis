...
if __name__ == '__main__':
    ...
    parser.add_argument('--video', type=str, help='video for unit testing')
    ...
    w = 200   # size of the tamper validation window (length)   (w > t_fix)

    cap = cv2.VideoCapture(args.video)  # put your video here
    frame_no = 0
    while True:
        frame_no += 1
        ...
        if frame_no > 0:
            ...
            cv2.imshow('frame_rgb', frame)
            cv2.moveWindow('frame_rgb', 500, 50)
            fgmask = handler.fgbg.apply(frame, learningRate = 0.002)
            fgmask = cv2.erode(fgmask, np.ones((1,1)))
            ...
            if tamper_flag == 1:    # if tamper attack occur:
                ...
                bg = handler.fgbg.getBackgroundImage()
            ...
            if frame_no > 1:  # not calculate anything in first frame
                mask_to_exclude = (fgmask == 255)

                e_bg = handler.sobel_edge_detection(background_gray)
                cv2.imshow('bg_sobel', e_bg)
                cv2.moveWindow('bg_sobel', 100, 400)
                e_c = handler.sobel_edge_detection(frame_gray)
                cv2.imshow('c_sobel', e_c)
                cv2.moveWindow('c_sobel', 500, 400)

                # th
                th = handler.calculate_th(e_bg)
                ...
                if frame_no > t_fix:    # generating background before monitoring
                    # tamper validation count:
                    tamper_validation_count = handler.calculate_tamper_validation_count(tamper_validation_count, w, edr, aedr, th)
                    ...
                    if tamper_validation_count > 30:
                        fixed_tamper_flag = True

                print(f'frame {frame_no}:\t tampered: {tamper_flag} \t valid tampers: {tamper_validation_count} \t status: {"Tamper attacked" if fixed_tamper_flag else "Normal"}')

                handler.aedr_list.append(aedr)
                handler.edr_list.append(edr)
                handler.aedr_th_list.append(aedr + th)
                handler.th_list.append(th)
